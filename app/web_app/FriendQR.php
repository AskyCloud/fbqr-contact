		...
		$(document).ready(function() { 
			...
			$('#form1').ajaxForm({ 
				...
				success: function(responseXML) {
					...
					if(stat=="none"){
						$('div#displayqr').fadeIn('slow');  
						$('div#displayqr').html('<div style="padding:5px"  class="fberrorbox"><center>Choose data</center></div>') ;
					}
					else if(stat=="fail"){
						$('div#displayqr').fadeIn('slow');  
						$('div#displayqr').html('<div style="padding:5px"  class="fberrorbox"><center>Fail</center></div>') ;
					}
					else if(stat=="no uid"){
						$('div#displayqr').fadeIn('slow');  
						$('div#displayqr').html('<div style="padding:5px"  class="fberrorbox"><center>Choose friend.</center></div>') ;
					}
					else if(stat=="done"){
...
echo "<form id=form1 name=form1 method=post action=xmlqr.php>";
	...
	if($numrow_uid==1){$c++;
		...
		echo "<tr>";
		echo "<td align=center  valign=top width=35px id=\"bb\" width=15px><input type=radio name=uid id=uncheck$c  onClick=\"return false\" onMouseDown=\"rC(this)\"  value=\"".$i['uid']."\"></td>";
		echo "<td  align=left  valign=bottom id=\"bb\" ><a href=\"friendinfo.php?fr=".$i['uid']."\" id=various$c target=blank >";
		echo "<img src=\"https://graph.facebook.com/".$i['uid']."/picture\" border=0 height=30px width=30px>";
		echo "<div id=line><b>".$i['first_name']."</b></div>";
		echo "</a></td></tr>";
	}
	}
	?>	
