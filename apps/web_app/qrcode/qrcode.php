...
class QRCode {
	...
	function makeImpl($test, $maskPattern) {
		...
		if($logosize!=0)
			$this->setupPositionLogoPattern(($this->moduleCount-$logosize)/2,($this->moduleCount-$logosize)/2);
	...
	function setupPositionLogoPattern($row, $col) {
		$logosize=$this->logosize;
		for ($r = -1; $r <= $logosize; $r++) {

			for ($c = -1; $c <= $logosize; $c++) {
    ...
    function getMinimumQRCode($data, $errorCorrectLevel,$size=0) {
